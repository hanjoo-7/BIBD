...
public class ImgGalleryDAO {
	...
	// 이미지 업로드
	public boolean uploadContent(String name,String p, int c) {
	...
	public boolean delimg(String a, int c) {
		sql = "delete from content where content_title = ? and couple_seq = ?";
		connectDB();
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, a);
	...
	public boolean updateimg(String s1, String s2, int c) {
		sql = "update content set content_title = ? where content_title = ? and couple_seq = ?";
		connectDB();
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<String> getlist(int c) {
		...
		try {
			sql = "select * from content where couple_seq = ?";
			connectDB();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, c);
	...
	public Image getIMG(String selected , int c) {
		InputStream input;
		FileOutputStream output;
		Image img = null;
		try {
			sql = "select * from content where content_title = ? and couple_seq = ?" ;
			connectDB();
			pstmt = conn.prepareStatement(sql);
